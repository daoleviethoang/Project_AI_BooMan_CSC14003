...
def check_fence(map, size, start):
    new_map = map
    flag = 0
    row_one = np.ones(size[0], dtype=int)
    col_one = np.ones(size[1], dtype=int).reshape((size[1], 1))
    ...
    if flag != 4:
        #them hang2 rao
        row_one = np.ones(size[0]).reshape((1, size[0]))
        col_one = np.ones(size[1] + 2).reshape((size[1] + 2, 1))
...
if __name__ == "__main__":
    size, map, start = readfile('input/2/input1.txt')
    size = np.array(size)
    map = np.array(map)
